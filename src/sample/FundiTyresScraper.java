...
public class FundiTyresScraper {
		...
		for (int page = 1; page <= MAX_PAGE; page++) {
			...
			for (int itemIndex = 0; itemIndex < productElements.size(); itemIndex++) {
				...
				String imageURL = element.findElement(
						By.xpath("./a[contains(@class, 'product-image')]/img"))
						.getAttribute("src");

				TyresCollection tyresCollection = new TyresCollection(null,
						productUrl, imageURL);
	...
	public void doUpdateTyres(TyresCollection tyresCollection) {
		...
		String productName = webDriver
				.findElements(
						By.xpath("//div[contains(@class, 'product-shop')]//div[@class='product-name']/h1"))
				.get(0).getText().trim();
		...
		String tyreSI = "";
			...
			.findElements(
					By.xpath("//table[@id='product-attribute-specs-table']//tr"));
			
			for (WebElement rowElement : rowElements) {
				String key = rowElement.findElement(By.xpath("./td[position() = 1]")).getText().trim().toLowerCase();
				String value = rowElement.findElement(By.xpath("./td[position() = 2]")).getText().trim();
				switch (key) {
				case "product sku":
					sku = value;
					break;
...
//		String sku = webDriver
//				.findElements(
//						By.xpath("//table[@id='product-attribute-specs-table']//tr[position() = 1]/td[position() = 1]"))
//				.get(0).getText().trim();
//		String branch = webDriver
//				.findElements(
//						By.xpath("//table[@id='product-attribute-specs-table']//tr[position() = 2]/td[position() = 1]"))
//				.get(0).getText().trim();
//		String width = webDriver
//				.findElements(
//						By.xpath("//table[@id='product-attribute-specs-table']//tr[position() = 4]/td[position() = 1]"))
//				.get(0).getText().trim();
//		String profile = webDriver
//				.findElements(
//						By.xpath("//table[@id='product-attribute-specs-table']//tr[position() = 5]/td[position() = 1]"))
//				.get(0).getText().trim();
//		String rimSize = webDriver
//				.findElements(
//						By.xpath("//table[@id='product-attribute-specs-table']//tr[position() = 6]/td[position() = 1]"))
//				.get(0).getText().trim();
//		String loadIndexSI = webDriver
//				.findElements(
//						By.xpath("//div[@class='tyre-specs-inner']//span[@class='tyre-li-se']"))
//				.get(0).getText().trim();
		...
		if (loadIndexSI.length() < 2) {
			...
			loadIndex = loadIndexSI.substring(0, loadIndexSI.length() - 1); // remove
																			// last
																			// SI
																			// character
		}

//		String tyreSI = webDriver
//				.findElements(
//						By.xpath("//table[@id='product-attribute-specs-table']//tr[position() = 7]/td[position() = 1]"))
//				.get(0).getText().trim();
		String price = webDriver
				.findElements(
						By.xpath("//div[@class='tyre-specs-inner']//span[@class='boldprice']"))
				.get(0).getText().trim();
	...
	public static void main(String[] args) throws IOException {
		...
		List<TyresCollection> tyresCollectionList = scrapper
				.getTyresCollectionList();

		ExcelUtil.marshalToXML(tyresCollectionList,
				"FundiTyresParsedAllUrl.xml");
