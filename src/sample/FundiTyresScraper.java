...
public class FundiTyresScraper {
	...
	private static final int MAX_PAGE = 184;
		...
		for (int page = 1; page <= MAX_PAGE; page++) {
			...
			for (int itemIndex = 0; itemIndex < NO_TYRES_TEST; itemIndex++) {
				...
				String productUrl = element.findElement(
						By.xpath("./a[@class='product-image']"))
						.getAttribute("href");

				String imageURL = element.findElement(
						By.xpath("./a[@class='product-image']/img")).getAttribute("src");
	...
	public void doUpdateTyres(TyresCollection tyresCollection) {
		...
		String productName = webDriver.findElements(By.xpath("//div[@class='product-shop']//div[@class='product-name']/h1")).get(0).getText().trim();
		...
		if (loadIndexSI.length() < 2) {
			...
			loadIndex = loadIndexSI.substring(0, loadIndex.length() - 1); // remove last SI character
		}
